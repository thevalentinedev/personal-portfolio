...
function carousel_setup() {
	...
	// Award descriptions in order of your carousel items
	...
	$('#carousel-description').text(awardDescriptions[0]);

	// Update description when carousel changes
	$('.image-carousel').on('afterChange', function (event, slick, currentSlide) {
		$('#carousel-description').text(awardDescriptions[currentSlide]);
	});
...
function lightbox_setup() {
	...
	$(".work-lightbox").each(function () {
		...
		lightbox.on('open', function () {
			...
			// 🔥 Force-hide the images immediately
...
document.addEventListener("DOMContentLoaded", function () {
	const sunIcon = "assets/images/sun.svg";
	const moonIcon = "assets/images/moon.svg";

	const iconImg = document.createElement("img");
	iconImg.id = "theme-icon";
	iconImg.alt = "Theme Icon";
	iconImg.style.width = "18px"; // Adjust size to match design
	...
	// FUNCTION: Update profile image source dynamically
	function updateProfileImageConfig() {
		if (document.body.classList.contains('light-mode')) {
			config_profile_image_url = config_profile_image_url_light;
		} else {
			config_profile_image_url = config_profile_image_url_dark;
		}

		const profilePic = document.getElementById('profile-pic');
		if (profilePic) {
			profilePic.src = config_profile_image_url;
		}
	}

	function refreshProfilePictureEffect() {
		const pictureBox = document.querySelector('.left-content .picture-box .picture');
		const parentBox = document.querySelector('.picture-box');

		if (pictureBox && parentBox) {
			// 1. Update background image
			$(pictureBox).css("background-image", "url(" + config_profile_image_url + ")");

			// 2. Destroy previous canvas if any (optional if multiple canvases stacking up)
			const oldCanvas = parentBox.querySelector('canvas');
			if (oldCanvas) {
				oldCanvas.remove();
			}

			// 3. Re-run the WebGL effect
			var map_url = 'assets/images/effect_maps/' + config_profile_image_effect + '.jpg';
			if (config_profile_image_effect == "custom") map_url = config_profile_image_effect_url;
	...
	// FUNCTION: Update background and bubbles based on mode
	function updateBackgroundComponents() {
		if (document.body.classList.contains('light-mode')) {
			$(".bg-color").css("background-color", "#f7f7f7");
			$(".bg-bubbles").removeClass("white").addClass("black");
		} else {
			$(".bg-color").css("background-color", "#181818");
			$(".bg-bubbles").removeClass("black").addClass("white");
		}
	}

	// 1. Check and apply saved theme preference
	...
	toggle.innerHTML = ''; // Clear old icon span
	toggle.appendChild(iconImg);

	// Call updates after loading
	updateProfileImageConfig();
	...
	toggle.addEventListener('click', () => {
		...
		iconImg.src = isLightMode ? moonIcon : sunIcon;
		localStorage.setItem('theme', isLightMode ? 'light' : 'dark');

		updateProfileImageConfig();
		updateBackgroundComponents();
		refreshProfilePictureEffect();
	});
});
